...
class ProjectEvent extends BaseProjectEvent {
	 ...
	 * @return void
	 */
	function notifyEventOrganizer($invitation_state, $responder = null) {
		if (!($responder instanceof Contact)) {
			$responder = logged_user();
		}
		$organizer = $this->getOrganizer();
		// if the event has no organizer or the responder is the organizer, do nothing
		if (!$organizer || !$responder || $organizer->getId() == $responder->getId()) return;
		...
		// send notification as the invitation contact (may differ from logged_user when
		// responding via shared mailbox access)
		Notifier::notifyEventWithIcs($this, [$organizer], 'invitation-'.$invitation_state_text, $responder);
	}
