<?php $genid = gen_id();?>

<form class="internalForm" action="<?php echo Comment::getAddUrl($comment_form_object) ?>" id="<?php echo $genid?>postcomment" method="post" enctype="multipart/form-data">
<?php tpl_display(get_template_path('form_errors')) ?>


  <div class="postComment contact-picture-container d-flex flex-start flex-wrap gap-2" id="<?php echo $genid?>_write_comment_div">
  	<img class="commentUserAvatar" src="<?php echo logged_user()->getPictureUrl()?>" alt="<?php echo logged_user()->getObjectName()?>"/>
	<div class="formAddCommentText">
    	<?php echo textarea_field("comment[text]", '', array('id' => $genid.'addCommentText', 'placeholder' => lang('write comment'))) ?>
	</div>
	<div class="button-container" id="<?php echo $genid?>_write_comment_btn">
		<?php echo submit_button(lang('add comment'), 's', array('id' => 'pcs' . $genid, 'class' => 'blue')) ?>
	</div>
	<div class="clear"></div>
  </div>
</form>
<script>
var genid = '<?php echo $genid?>';

$(function() {
	$('#'+genid+'addCommentText').keyup(function(){
		var max_height = 2000;
		if ($(this)[0].scrollHeight > $(this)[0].clientHeight && $(this)[0].clientHeight < max_height) {
			var h = $(this)[0].scrollHeight + 10;
			if (h > max_height) {
				h = max_height;
				$(this).css('overflow-y', 'auto');
			}
			
			$(this).css('height', h +'px');
		} else {
			if ($(this)[0].clientHeight > max_height) {
				$(this).css('overflow-y', 'auto');
			}
		}
	});
	

	// disable comment button after submit
	$("#"+genid+"postcomment").submit(function () {
		var text = $('#'+genid+'addCommentText').val();
		if (text != "") {
			// disable the button and enable it 2 seconds later
			$("#"+genid+"_write_comment_btn button").attr("disabled", "disabled");
			setTimeout(function() {
				$("#"+genid+"_write_comment_btn button").removeAttr('disabled');
			}, 2000);
		}
        return true;
    });
});
</script>
