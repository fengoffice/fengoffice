...
array_unshift($ordered_properties, $role_property);
...
foreach ($ordered_properties as $property) {
	if (in_array($property['id'], $properties_to_show)) {
		...
		if ($property['id'] == 'name') {
			...
			$property_value = $contact->getUserTypeName();
			...
			if (str_starts_with($property['id'], 'cp_')) {
				...
				if ($property_value instanceof DateTimeValue) {
					$property_value = format_date($property_value);
				}
			}
			$property_class = 'other-property';
		}

		if (trim((string)$property_value) != '') {
			$properties_data[] = array(
				'id' => $property['id'],
				'name' => $property['name'],
...
<?php if ($show_only_name) { ?>
	<div class="selected-object-wrapper">
		<div class="object-badge">
			<i class="icon-user"></i>
			<span class="name" title="<?php echo clean($contact->getObjectName()); ?>"><?php echo clean($contact->getObjectName()); ?></span>

			<a href="#"
			   tabindex="-1"
			   onclick="<?php echo $on_remove; ?>;this.parentElement.parentElement.remove();"
...
<div class="contact-selector-card" id="contact-selector-card-<?php echo $hf_id . '-' . $contact_id; ?>">
	...
	<div class="section contact-actions">
		<a href="#" tabindex="-1" class="link-ico ico-delete" onclick="<?php echo $on_remove; ?>"></a>
	</div>
