...
<?php
	...
	foreach ($cal_subtasks['dated'] as $subtask) {
		$tmp_tasks = array_merge($tmp_tasks, replicateRepetitiveTaskForCalendar($subtask, $date_start, $date_end));
	}
	...
	if ($show_subtasks && !empty($cal_subtasks['undated_by_parent'])) {
		foreach ($tasks as $task) {
			...
			$instances = replicateRepetitiveTaskForCalendar($task, $date_start, $date_end);
