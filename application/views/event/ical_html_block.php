<div class="calendar-container">
	...
	<div class="event">
		...
		<?php if (!empty($show_confirm_attendance[$i])) : ?>

		<div class="confirm-attendance">
			<h4><?php echo lang('confirm attendance'); ?>:</h4>
			<div class="event-invitation-buttons"
				data-event-uid="<?php echo clean($event['uid']) ?>"
				data-user-id="<?php echo (int) array_var($invitation_contact_ids, $i, logged_user()->getId()) ?>"
				<?php if (!empty($mail_account_id)) : ?>data-mail-account-id="<?php echo (int) $mail_account_id ?>"<?php endif; ?>>
...
$('.event-invitation-buttons .confirm-att-btn').click(function() {
	...
	var $btns = $(this).closest('.event-invitation-buttons');
	var event_uid = $btns.attr('data-event-uid');
	var user_id = $btns.attr('data-user-id');
	var mail_account_id = $btns.attr('data-mail-account-id');
	var post = {
	...
	if (mail_account_id) {
		post.mail_account_id = mail_account_id;
	}
	...
	og.openLink(og.getUrl('event', 'change_invitation_state'), {
		...
		callback: function(success) {
			if (success) {
				$btns.find('.confirm-att-btn').css('opacity', '0.5');
				$(this).css('opacity', '1').css('font-weight', 'bold');
			}
		}.bind(this)
	});
});
