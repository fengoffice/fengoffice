    ...
    if ($contact_data_type) {
        ...
        switch ($typeObject->getName()) {
            case "contact":
                ...
                if (!$existingContact && trim($contactEmail) != "") {
                    if (config_option('allow_duplicated_contact_emails')) {
						...
						if (is_array($contactEmails) && count($contactEmails) > 0) {
				...
				// Never associate the member to a user. After this, the if/elseif/else
				// below only assign a non-user $existingContact or new Contact().
				if ($existingContact instanceof Contact && $existingContact->isUser()) {
					$existingContact = null;
				}
                ...
                // Unreachable with the current lookup: isUser() already nulls
                // $existingContact, so $contact is always a non-user or isNew().
                // Kept so a later assignment of $contact cannot run setFromAttributes
                // / setFirstName on a user.
                if ($contact instanceof Contact && !$contact->isNew() && $contact->isUser()) {
                    $contact = new Contact();
                }
            ...
            case "company":
                ...
                $existingContact = null;
                $companyEmail = trim(array_var($companyData, 'email', ''));

                if (!$isNew && $object->getContactId() > 0) {
                    $existingContact = Contacts::instance()->findById($object->getContactId());
                }

                // This lookup used to be dead (isset($contactEmail) in the company
                // case, and $contactEmail is never set here). It now reuses an
                // existing *company* with the same email instead of always creating
                // a duplicate. Users and person contacts that share the address are
                // skipped; a new company is created in those cases.
                if (!$existingContact && $companyEmail != "") {
                    $contactEmails = ContactEmails::instance()->findAll(array(
                        'conditions' => array("`email_address` = ?", $companyEmail)
                    ));
                    foreach ($contactEmails as $ce) {
                        if (!can_read_sharing_table(logged_user(), $ce->getContactId())) {
                            continue;
                        }
                        $found = Contacts::instance()->findById($ce->getContactId());
                        if ($found instanceof Contact && $found->getIsCompany() && !$found->isUser()) {
				...
				// Never reuse a user as the company record of a member (supplier/customer/etc.).
				// Covers the already-linked contact_id path; email lookup above already skips users.
				if ($existingContact instanceof Contact && $existingContact->isUser()) {
					$existingContact = null;
				}
                ...
                // Unreachable with the current lookup: isUser() already nulls
                // $existingContact, so the branches above only assign a non-user
                // or new Contact() ($company->isNew() === true). Kept so a later
                // assignment of $company cannot run setFirstName/setIsCompany on a user.
                if ($company instanceof Contact && !$company->isNew() && $company->isUser()) {
                    $company = new Contact();
                }
