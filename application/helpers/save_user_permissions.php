<?php
	...
	// save permissions
	// argv[4] is a spool file listing every permission group queued by the originating request,
	// so one process now covers what used to be one process per permission group. Claim it first:
	...
	foreach ($jobs as $job) {
		$pg_id = array_var($job, 'pg_id');
		$is_guest = array_var($job, 'is_guest');
		$permissions_filename = array_var($job, 'permissions_filename');
		$sys_permissions_filename = array_var($job, 'sys_permissions_filename');
		$mod_permissions_filename = array_var($job, 'mod_permissions_filename');
		$root_permissions_filename = array_var($job, 'root_permissions_filename');
		$users_ids_to_check_filename = array_var($job, 'users_ids_to_check_filename');
		$root_permissions_genid = array_var($job, 'root_permissions_genid');
		$only_member_permissions = array_var($job, 'only_member_permissions') == "1";
		$is_new_user = array_var($job, 'is_new_user') == "1";

		// The payload files are deleted once their permission group is done, so a missing file
		// means this job already ran and the batch is being retried after a crash. Re-running it
		// would feed save_permissions() empty payloads and rewrite the group from nothing.
		...
		try {
			...
			// One failing permission group must not cost the remaining ones in the batch.
			Logger::log("Error saving user permissions for permission group $pg_id: ".$e->getMessage()."\n".$e->getTraceAsString());
		}
	}
