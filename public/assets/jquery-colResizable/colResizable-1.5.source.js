...
(function($){ 	
	...
	var createGrips = function(t){	
		...
		th.each(function(i){						//iterate through the table column headers			
			var noresize = 'style="display: none;"';
			if($(this).data('resizable')){			//if this th have data-resizable allow resize
				noresize = '';
			}
			var c = $(this); 						//jquery wrap for the current column			
			var g = $(t.gc.append('<div class="JCLRgrip" '+noresize+'></div>')[0].lastChild); //add the visual node to be used as grip
	...
	var memento = function(t, th){ 
		...
		if(th){										//in deserialization mode (after a postback)
			...
			for(;i<t.ln;i++){						//for each column
				if(th.eq(i).data('resizable')){
					aux.push(w[i]+PX); 
					//aux.push(100*w[i]/tw+"%"); 	//width is stored in an array since it will be required again a couple of lines ahead
				}else{
					aux.push(th.eq(i).css('min-width'));
				}
			...
			for(i=0;i<t.ln;i++){
				...
				if(th.eq(i).data('resizable')){
				//	t.cg.eq(i).width(t.cg.eq(i).width()+PX);
					t.cg.eq(i).css("min-width", aux[i]);
					th.eq(i).css("min-width", aux[i] );
				}
			}
	...
	*/
	var applyBounds = function(t){
        ...
        $.each(t.c, function(i,c){
        	if(c.data('resizable')){			//if this th have data-resizable allow resize
				c.width(w[i]).w = w[i];
			}
	...
	var onResize = function(){
		...
		if (t instanceof jQuery){			
			...
			t.removeClass(FLEX);					//firefox doesn't like layout-fixed in some cases
