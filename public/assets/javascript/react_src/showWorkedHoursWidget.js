...
var { DateFilterWidgetBase, installWidgetOptionSaver } = require('./helpers/widgetDateFilterHelpers');
...
class WorkedHoursWidget extends React.Component {
    constructor(props) {
        ...
        installWidgetOptionSaver(this.dateFilterHelper, 'estimated_worked_time', 'worked_hours_widget', props.data.saveWidgetOptionUrl);
    ...
    parseItemDay(item) {
        if (item.dateKey) {
            return moment(item.dateKey, 'YYYY-MM-DD');
        }
        return moment(item.date, this.state.dateFormat);
    }
    ...
    render() {
        ...
        if(chartData){
            chartData.forEach(d => {
                const m = d.dateKey
                    ? moment(d.dateKey, 'YYYY-MM-DD').startOf('day')
                    : moment(d.date, dateFormat).startOf('day');
                d.date = m.valueOf();
            });
        }
